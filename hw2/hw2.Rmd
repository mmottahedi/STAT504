...
X 1 , X 2 , ..., X n be the number of Mega Millions winners in the first n weeks of
2010.
...
The values of $\chi^2$ and $G^2$ are not large so the model is not that bad but we cannot say that the sample is  from the eligible population.
...
Perform the X 2 and G 2 goodness-of-fit tests to see if these data are Poisson. The last
category ( 5), indicates that outcomes where the count was 6 and higher were rare so
we collapsed them to the category (x  5). Also compute and explain the MLE of the
